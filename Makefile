...
# ============================================================
#  Pipeline Local (el flujo principal)
# ============================================================
preprocess: ## Preprocesa el dataset de Liga MX
	$(PYTHON) src/training/preprocess.py

train: ## Entrena el modelo localmente
	$(PYTHON) src/training/train.py

predict: ## Predicción rápida (uso: make predict HOME="América" AWAY="Guadalajara")
	$(PYTHON) -c "\
		from src.inference.predict import predict, load_artifacts; \
		load_artifacts(); \
		r = predict('$(HOME)', '$(AWAY)'); \
		print(f\"\n⚽ {r['home_team']} vs {r['away_team']}\"); \
		print(f\"🏆 Predicción: {r['label']}\"); \
		[print(f\"   {k}: {v:.1%}\") for k, v in r['probabilities'].items()]"

pipeline: preprocess train ## Ejecuta pipeline completo (preprocess + train)
...
docker-test: docker-build ## Prueba la imagen localmente
	@echo '{"body": "{\"home_team\": \"América\", \"away_team\": \"Guadalajara\"}"}' | \
		docker run --rm -i -v $(PWD)/models:/tmp $(ECR_REPO)-inference:latest

# ============================================================
#  Deploy a AWS (solo al final del proyecto)
# ============================================================
docker-push: ## Sube imagen a Amazon ECR
...
deploy: docker-push ## Deploy completo: sube modelo a S3 + imagen a ECR + update Lambda
